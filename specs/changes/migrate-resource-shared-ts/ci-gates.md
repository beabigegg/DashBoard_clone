...
contract-version: 1.3.3
last-updated: 2026-05-12
---

# CI/CD Gate Plan — migrate-resource-shared-ts

Governed by `contracts/ci/ci-gate-contract.md` schema 1.3.3.
...
| frontend-legacy | `cd frontend && npm run test:legacy` | 244/244 legacy node --test assertions pass (incl. `resource-status.test.js`) | PASSED |
| css-governance | `cd frontend && npm run css:check` | 0 new violations (47 pre-existing warnings unchanged) | PASSED |
| frontend-build | `cd frontend && npm run build` | Vite exits 0; no resolution failures (11.20s) | PASSED |
| contract-validate | `cdd-kit validate` | 0 contract violations | PENDING |
| cdd-strict-gate | `cdd-kit gate migrate-resource-shared-ts --strict` | 0 open tasks, all AC green | PENDING |
...
| gate | command | pass criteria | status |
|---|---|---|---|
| frontend-type-check | `cd frontend && npm run type-check` | 0 type errors across `src/core/**/*`, `src/shared-composables/**/*`, `src/shared-ui/**/*`, `src/admin-shared/**/*`, `src/resource-shared/**/*` (scope expanded in this phase) | PASSED |
...
PR is eligible to merge once all required gates (Tier 1 PENDING) pass in CI. The `frontend-type-check` gate remains **informational** per contract 1.3.3 — its scope expanded to include `src/resource-shared/**/*` in this PR but promotion to required follows the standard Informational Gate Promotion Policy (20 days / 60 runs).

## Workflow

Governed by `.github/workflows/frontend-tests.yml` (frontend-unit, frontend-legacy, css-governance, frontend-type-check, frontend-build) and `.github/workflows/contract-driven-gates.yml` (contract-validate, cdd-strict-gate, playwright gates).
