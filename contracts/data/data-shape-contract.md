...
| models (機型, UPH-06) | `EXISTS (SELECT 1 FROM DWH.DW_MES_RESOURCE r WHERE r.RESOURCENAME = e.EQUIPMENT_ID AND r.OBJECTCATEGORY = 'ASSEMBLY' AND NVL(TRIM(r.RESOURCEFAMILYNAME),'(NA)') IN (...))` | DWH.DW_MES_RESOURCE.RESOURCEFAMILYNAME | EXISTS semi-join (mirrors workcenter_names) |
...
| MODEL | VARCHAR | yes | `DW_MES_RESOURCE.RESOURCEFAMILYNAME` (real 機型 e.g. `DBA_AD832UR`) via the same bridge (v2, UPH-06); `NULL` if no resource match. Groupable trend dimension + detail column |
...
**Detail** (`GET /api/uph-performance/detail`): `data = {rows: [{lot_id: string, equipment_id: string, model: string | null, event_time: string (ISO 8601), uph_value: number | null, package: string | null, pj_type: string | null}], meta: {page: int, per_page: int, total_count: int, total_pages: int}}`. `model` (機型, `RESOURCEFAMILYNAME` — v2, UPH-06) is `null` when the equipment has no `DW_MES_RESOURCE` match. Pagination: `per_page` max 200 (mirrors EA-04 detail pagination).
...
Constraints: unique constraint on `raw_package_lf`. **Default-on-absence (D1 — sparse, fallback-to-self):** a raw `PACKAGE_LF` value with NO row in this table is NOT excluded from the report — it groups under itself (its own raw value becomes its own `package_lf_group`). Client-side resolution is a `LEFT JOIN` (never `INNER JOIN`) plus `COALESCE(merged_group, raw_package_lf, '(未分類)')` — NULL/blank raw `PACKAGE_LF` resolves to the sentinel group `"(未分類)"`. Every raw value Oracle has ever emitted appears in the report under some group — new raw values that appear in Oracle in the future require zero code or table changes to show up (they simply appear as their own group until an admin explicitly adds a merge row). When `MYSQL_OPS_ENABLED=false`, the CRUD read endpoint (`get_package_lf_entries()`) degrades to an empty array (never 500). **Oracle default layer (change `production-achievement-oracle-plan-source`, business-rules.md PA-09 amendment):** the inline `package_lf_map` (§3.33) is NO LONGER sourced from this table alone — `get_package_lf_map()` starts from `DWH.MES_WIP_OUTPUTPLAN_DETAIL`'s full PACKAGE→PACKAGE_REPORT reference table (cached, TTL `CACHE_TTL_PRODUCTION_ACHIEVEMENT_PACKAGE_LF_ORACLE`) as the default, then overlays this table's manual rows on top (manual always wins). NOT `MES_WIP_OUTPUTPLAN_DETAIL.PACKAGEGROUP` — that is a different, coarser internal planning grouping on the same table that diverges from `PACKAGE_REPORT` for several raw codes (confirmed via live-data cross-check, e.g. `PACKAGE='SOT-23 CU'` has `PACKAGEGROUP='SOT-23'` but `PACKAGE_REPORT='SOT-23 CU'`); using the wrong column was a genuine bug caught during user review, fixed before this table's Oracle-sourced default layer shipped. So `MYSQL_OPS_ENABLED=false` no longer empties `package_lf_map` — it still reflects the Oracle default layer; only a manual override row is lost. `package_lf_map` is empty only if BOTH this table AND the Oracle read degrade (Oracle failure alone degrades to {} / last-known-good, never raises). Write endpoints return 503 `SERVICE_UNAVAILABLE` when MySQL OPS is disabled (same as §3.26/§3.27).
...
| plan_source_side | string (`input` \| `output`) | yes |
| updated_at | string | yes |
| updated_by | string | yes |

Notes: `id` is the primary key (auto-increment); not exposed via the API response row shape (`ProductionAchievementWorkcenterMergeMapRow`, api-contract.md) — `DELETE` identifies a row by `raw_workcenter_group` in the URL path. `raw_workcenter_group` is a `WORK_CENTER_GROUP`/`FROMWORKCENTER` value; unique. `merged_workcenter_group` is the display group name (子站). `parent_group` (added by change `production-achievement-moveout`, business-rules.md PA-19) is the 大項 this子站 rolls up under — `= merged_workcenter_group` for every single-layer station; only `電鍍` (parent of 掛鍍/條鍍/滾鍍/委外) and `切割` (parent of 切割/PKG_SAW) differ. The station dropdown (`get_filter_options().workcenter_groups`) is the DISTINCT `parent_group` set; a大項 with >1子站 renders the detail table expanded per子站. A legacy NULL `parent_group` (pre-backfill install) falls back to `merged_workcenter_group` in the service read. The migration is an idempotent `ALTER TABLE … ADD COLUMN parent_group` + backfill in `scripts/sql/production_achievement_tables.sql`. `plan_source_side` (added by change `production-achievement-oracle-plan-source`, business-rules.md PA-20) is which Oracle `MES_WIP_OUTPUTPLAN` column (`Planqty_Input` vs `PLANQTY_OUTPUT`, §3.34) this 大項 sources its target from — DDL default `'input'`; a legacy NULL/blank value (pre-migration install) falls back to `'input'` in the service read, same pattern as `parent_group`'s own backfill. REQUIRED (not optional) on every `PUT` — always submitted together with `parent_group` in the SAME write, never independently, so a 大項 reassignment can never silently leave a stale routing.
...
### §3.32 Production-Achievement Daily-Plan Table (MySQL, `production_achievement_daily_plans`) — **DEPRECATED, superseded by §3.34**

Added by change `production-achievement-overhaul`; **superseded by change `production-achievement-oracle-plan-source`** — see business-rules.md PA-11/PA-16. Targets are now sourced from Oracle `MES_WIP_OUTPUTPLAN` (§3.34, `plan_map`), NOT this table. This table is NOT dropped (existing rollback policy, `scripts/sql/production_achievement_tables.sql`'s own "do NOT DROP" note) but no code path reads or writes it anymore — the schema below is kept for historical reference only.
...
Notes (historical): `id` was the primary key (auto-increment). `workcenter_group` matched a `merged_workcenter_group` value produced by §3.31. `package_lf_group` matched a `merged_group` value produced by §3.30 (or a raw fallback-to-self value, per D1). `daily_plan_qty` was a non-negative integer, the denominator for business-rules.md PA-12/PA-13 before those rules were rewritten to read §3.34 instead. No `output_date` column — this table was explicitly date-independent (the same value reused for every calendar day), which is exactly the limitation §3.34's date-indexed replacement fixes.

Constraints (historical): unique constraint on `(workcenter_group, package_lf_group)`; writes were upsert (`INSERT ... ON DUPLICATE KEY UPDATE`). The `GET/PUT /api/production-achievement/daily-plans` and `POST /api/production-achievement/daily-plans/import/preview`+`/import/confirm` endpoints that read/wrote this table no longer exist (removed, api-contract.md).
...
Notes: FULL current mapping table content. A rolled-up `workcenter_group` (from Stage 1, `spec_workcenter_map`) with no matching row here IS excluded from the final rollup — client resolves via `INNER JOIN` (D2, business-rules.md PA-10) — the OPPOSITE join kind from `package_lf_map` above; see ADR-0016's Extension section. When `MYSQL_OPS_ENABLED=false`, this array is empty — the INNER JOIN then matches nothing and the report renders empty for every workcenter_group (D2's own default, not an error state; contrast with `package_lf_map`'s empty-array behavior above). `plan_source_side` (business-rules.md PA-20, added by change `production-achievement-oracle-plan-source`) lets the client resolve which §3.34 `plan_map` column (`planqty_input`/`planqty_output`) the selected 大項 reads.

### §3.34 `plan_map` (inline-injected, HTTP 200 spool-hit response) — replaces `daily_plan_map`

Added by change `production-achievement-oracle-plan-source` (business-rules.md PA-11), replacing the Excel-imported `daily_plan_map` (formerly sourced from the now-deprecated §3.32 table). Sourced from `services/production_achievement_plan_service.get_oracle_plan_rows()`, which reads Oracle `DWH.MES_WIP_OUTPUTPLAN` joined to `DWH.MES_WIP_OUTPUTPLAN_DETAIL` (`sql/production_achievement_plan.sql`) — NOT a MySQL table this application owns. Injected inline alongside the other maps (§3.28.2/§3.28.3/§3.33).

| field | type | required |
|---|---|---|
| output_date | string (`YYYY-MM-DD`) | yes |
| plan_package_group | string | yes |
| planqty_input | integer | yes |
| planqty_output | integer | yes |

Notes: One row per `(output_date, plan_package_group)` — Oracle's own grain, ONE ROW PER DAY PER PACKAGE (unlike the old date-independent `daily_plan_map`). `plan_package_group` is the merged Package Group name resolved via `MES_WIP_OUTPUTPLAN_DETAIL.PACKAGE_REPORT` (business-rules.md PA-11 — NOT `.PACKAGEGROUP`, a different, coarser column on the same table that diverges from `PACKAGE_REPORT` for several raw codes) — it must line up with the `merged_group` values produced by §3.30 (`production_achievement_package_lf_map`, D1) for the client-side join `pa_plan_map.plan_package_group = pa_rollup.package_lf_group` to match; a Package Group present in Oracle's plan data but with no corresponding §3.30 D1 mapping simply never matches (no target found, `achievement_rate = null` — same missing-row degrade posture as every other map here). The array ships the FULL calendar month(s) touched by the request's `[start_date, end_date]` range (not narrowed to the exact range, since the server-side cache is keyed per-month) — client-side `computeDailyView`/`computeCumulativeView` filter to the exact requested date(s) via `CAST(pm.output_date AS DATE) = ...`/`BETWEEN ...`. `planqty_input` (前中段/投入目標) and `planqty_output` (TMTT後/產出目標) are BOTH always present per row; which one a given render reads is resolved from the selected station/大項's `plan_source_side` (§3.33's `workcenter_merge_map`, PA-20) — never both, never per-row. `shift_plan_qty` (business-rules.md PA-21, `= CEIL(daily_plan_qty / 2)`) and `d_achievement_rate`/`n_achievement_rate` are DERIVED client-side, not part of this wire shape. On an Oracle read failure for a given month, that month's rows degrade to `[]` (never raises, never 500s `/report`) — every client-computed `achievement_rate` for dates in that month becomes `null`, same degrade posture the old `daily_plan_map` had for `MYSQL_OPS_ENABLED=false`.
