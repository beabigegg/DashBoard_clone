...
| EAP_ALARM_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; selects unified EapAlarmJob path (on, default) vs legacy run_eap_alarm_query_job (off). Restart required. | yes | uses default on |
| PRODUCTION_HISTORY_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; selects unified ProductionHistoryJob path (on, default) vs legacy pandas BQE path (off). Restart required. | yes | uses default on |
| REJECT_HISTORY_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; selects unified RejectHistoryJob path (on, default) vs legacy execute_primary_query path (off). Restart required. | yes | uses default on |
| RESOURCE_HISTORY_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; selects unified ResourceHistoryBaseJob+ResourceHistoryOeeJob path (on, default) vs legacy export_csv Oracle read path (off). Restart required. | yes | uses default on |
| MATERIAL_TRACE_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; selects unified MaterialTraceJob path (on, default) vs legacy _execute_batched_query_to_parquet path (off). always_async=True; async unavailable → 503 (no sync fallback). Restart required. | yes | uses default on |
| DOWNTIME_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; selects unified DowntimeJob path (on, default) vs legacy _bridge_jobid Path B pd.merge (off). always_async=True; async unavailable → 503 (no sync fallback). Restart required. | yes | uses default on |
| QUERY_TOOL_USE_RQ | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; enables RQ async dispatch for query-tool equipment-period endpoint when classify_query_cost returns ASYNC (date span ≥ 30 days). When on + oversized query → 202+job_id; else inline. Restart required. | yes | uses default on |
| PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; gates whether the `ProductionAchievementJob` worker/job-type is registered. Default `on`. Unlike the other `*_USE_UNIFIED_JOB` flags, there is NO legacy code path behind `off` — clean pre-launch replacement, no dual-path. `off` disables job-type registration; `/report` spool-miss then returns 503 (safe kill-switch, page unexposed pre-launch). `always_async=True`; `sync_fallback_allowed=False`. Restart required. | yes | uses default on; `/report` spool-miss returns 503 |
| UPH_PERFORMANCE_USE_UNIFIED_JOB | feature | all | no | no | on | on | application-team | off/on or false/true or 0/1; gates whether the `UphPerformanceJob` worker/job-type is registered. Default `on`. No legacy code path exists behind `off` (clean pre-launch replacement, mirrors `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB`) — `off` disables job-type registration; spool-miss on `/spool` then returns 503. `always_async=True`; `sync_fallback_allowed=False`. Restart required. | yes | uses default on; spool-miss returns 503 |

- `EAP_ALARM_USE_UNIFIED_JOB`: Feature flag selecting the unified `EapAlarmJob` path (`BaseChunkedDuckDBJob` template method). Default `on` (AC-1 parity tests all pass). When `on`/`true`/`1`, the eap_alarm route enqueues via `enqueue_query_job` + `EapAlarmJob`; the always-async 503 decision tree applies (`sync_fallback_allowed=False`). When `off`/`false`/`0`, falls back to legacy `run_eap_alarm_query_job`. Module-level constant frozen at import — restart required. `monkeypatch.setattr()` required in tests (not `setenv`). Added by change `eap-alarm-unified-job-poc`; default promoted to `on` 2026-06-20.
...
- `QUERY_TOOL_USE_RQ`: Feature flag enabling RQ async dispatch for the query-tool `POST /api/query-tool/equipment-period` endpoint (query-path-c-elimination-cleanup, P5). When `on`/`true`/`1` and `classify_query_cost(domain="query_tool", ...)` returns `ASYNC` (date span ≥ 30 days by default), the route enqueues via `enqueue_query_job("query-tool", ...)` and returns 202 + `{async: true, job_id, status_url}`. When `off` (default)/`false`/`0`, the route behaves exactly as before (inline sync). Fail-open: if RQ is unavailable or enqueue fails, falls through to sync path. Module-level constant `_QUERY_TOOL_USE_RQ` frozen at import — restart required. `monkeypatch.setattr()` required in tests. Added by change `query-path-c-elimination-cleanup`.
...
- `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB`: Feature flag gating registration of the `ProductionAchievementJob` worker/job-type (`BaseChunkedDuckDBJob`, `chunk_strategy=TIME`, `requires_cross_chunk_reduction=False` with a re-aggregating `post_aggregate` — ADR-0016). Default `on`. **Deviates from the naming convention of the other `*_USE_UNIFIED_JOB` flags above**: those select between a NEW unified-job path and a retained LEGACY path; `production-achievement-async-spool` is a clean pre-launch replacement of the prior synchronous `get_achievement_report()` Oracle path — no legacy path is reachable from the route. `off` therefore functions purely as an emergency kill switch: the job type is not registered at worker preload, so a spool-miss on `/report` finds no available job type and returns 503 `SERVICE_UNAVAILABLE` (same always-async decision tree as `EAP_ALARM_USE_UNIFIED_JOB`/`MATERIAL_TRACE_USE_UNIFIED_JOB`/`DOWNTIME_USE_UNIFIED_JOB`, but with no flag-off fallback branch). A spool-HIT `/report` request still succeeds (HTTP 200) even with the flag `off`, because it reads an already-written parquet without touching the worker. Module-level constant frozen at import — restart required. `monkeypatch.setattr()` required in tests. **Worker env-var parity**: the new `mes-dashboard-production-achievement-worker.service` systemd unit MUST export `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB` with the same value as gunicorn. Added by change `production-achievement-async-spool`.
- `UPH_PERFORMANCE_USE_UNIFIED_JOB`: Feature flag gating registration of the `UphPerformanceJob` worker/job-type (`BaseChunkedDuckDBJob`, `chunk_strategy=TIME`, ≤6h chunk windows per UPH-01). Default `on`. Like `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB`, this is a clean pre-launch replacement — no legacy code path exists behind `off`; `off` is a pure kill switch (job type not registered at worker preload; a spool-miss on `POST /api/uph-performance/spool` returns 503 `SERVICE_UNAVAILABLE`, same always-async decision tree as `EAP_ALARM_USE_UNIFIED_JOB`/`PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB`). A spool-HIT request still succeeds (HTTP 200) even with the flag `off`. Module-level constant frozen at import — restart required. `monkeypatch.setattr()` required in tests. **Worker env-var parity**: the new `mes-dashboard-uph-performance-worker.service` systemd unit MUST export `UPH_PERFORMANCE_USE_UNIFIED_JOB` with the same value as gunicorn. Added by change `add-uph-performance-page`.
...
- `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB` (production-achievement worker)
- `UPH_PERFORMANCE_USE_UNIFIED_JOB` (uph-performance worker)
...
**Worker env-var parity:** The `mes-dashboard-hold-history-worker.service` systemd unit MUST export the same `HOLD_*` env set as gunicorn (at minimum: `HOLD_ASYNC_ENABLED`, `HOLD_WORKER_QUEUE`, `HOLD_JOB_TIMEOUT_SECONDS`). Env-var drift silently changes query routing. Added by change `hold-history-rq-async`. `HOLD_ASYNC_DAY_THRESHOLD` removed by `query-path-c-elimination-cleanup` (P5); routing now uses `classify_query_cost(domain="hold", ...)` with the unified `CostPolicy`.
...
**Worker env-var parity:** The `mes-dashboard-resource-history-worker.service` systemd unit MUST export the same `RESOURCE_*` env set as gunicorn (at minimum: `RESOURCE_ASYNC_ENABLED`, `RESOURCE_WORKER_QUEUE`, `RESOURCE_JOB_TIMEOUT_SECONDS`). Env-var drift silently changes query routing. Added by change `resource-history-rq-async`. `RESOURCE_ASYNC_DAY_THRESHOLD` removed by `query-path-c-elimination-cleanup` (P5); routing now uses `classify_query_cost(domain="resource", ...)` with the unified `CostPolicy`.

<!-- REJECT_ASYNC_DAY_THRESHOLD removed by query-path-c-elimination-cleanup (P5); routing now uses classify_query_cost(domain="reject", ...) with the unified CostPolicy. -->
...
## Async Worker — Production Achievement Query

| name | scope | environments | required | secret | default | example | owner | validation | restart required | failure behavior |
|---|---|---|---:|---:|---|---|---|---|---:|---|
| PRODUCTION_ACHIEVEMENT_WORKER_QUEUE | async | all | no | no | production-achievement-query | production-achievement-query | application-team | non-empty string; RQ queue name for the production-achievement worker process | yes | uses default "production-achievement-query" |
| PRODUCTION_ACHIEVEMENT_JOB_TIMEOUT_SECONDS | async | all | no | no | 1800 | 1800 | application-team | positive integer (seconds); RQ job timeout for the production-achievement worker; must exceed the longest expected Oracle TIME-chunk fan-out duration | yes | uses default 1800 |

- `PRODUCTION_ACHIEVEMENT_WORKER_QUEUE`: RQ queue name that `enqueue_job_dynamic()` routes production-achievement jobs to. Must match the `--queues` argument of the running `mes-dashboard-production-achievement-worker.service` process. Default `"production-achievement-query"`. Added by change `production-achievement-async-spool`.
- `PRODUCTION_ACHIEVEMENT_JOB_TIMEOUT_SECONDS`: Maximum seconds a single RQ production-achievement job may run before the worker kills it. Default 1800 s, matching the other `BaseChunkedDuckDBJob` worker timeouts. Added by change `production-achievement-async-spool`.

**Worker env-var parity:** The new `mes-dashboard-production-achievement-worker.service` systemd unit MUST export the same `PRODUCTION_ACHIEVEMENT_*` env set as gunicorn (at minimum: `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB`, `PRODUCTION_ACHIEVEMENT_WORKER_QUEUE`, `PRODUCTION_ACHIEVEMENT_JOB_TIMEOUT_SECONDS`). Env-var drift silently breaks job-type registration parity. Added by change `production-achievement-async-spool`.

**No dedicated `PRODUCTION_ACHIEVEMENT_SPOOL_DIR`/`_SPOOL_TTL`**: unlike `EAP_ALARM_SPOOL_DIR`/`EAP_ALARM_SPOOL_TTL`, this feature reuses the shared `QUERY_SPOOL_DIR` base directory (namespace subdirectory `production_achievement/`, per `query_spool_store.py`'s per-namespace path resolution) and the shared `CACHE_TTL_DATASET` default for Redis spool-metadata TTL. Exact TTL tuning is deferred to implementation (design.md Open Decisions); introduce `PRODUCTION_ACHIEVEMENT_SPOOL_TTL` as a follow-up contract change only if the shared default proves unsuitable.

## Async Worker — UPH Performance Query

| name | scope | environments | required | secret | default | example | owner | validation | restart required | failure behavior |
|---|---|---|---:|---:|---|---|---|---|---:|---|
| UPH_PERFORMANCE_WORKER_QUEUE | async | all | no | no | uph-performance-query | uph-performance-query | application-team | non-empty string; RQ queue name for the uph-performance worker process | yes | uses default "uph-performance-query" |
| UPH_PERFORMANCE_JOB_TIMEOUT_SECONDS | async | all | no | no | 1800 | 1800 | application-team | positive integer (seconds); RQ job timeout for the uph-performance worker; must exceed the longest expected Oracle ≤6h-chunk fan-out duration (EAP_EVENT detail JOIN previously timed out >180s over a 24h window — see docs/architecture/eap-event-uph-collection-investigation.md) | yes | uses default 1800 |

- `UPH_PERFORMANCE_WORKER_QUEUE`: RQ queue name that `enqueue_query_job()` routes uph-performance jobs to. Must match the `--queues` argument of the running `mes-dashboard-uph-performance-worker.service` process. Default `"uph-performance-query"`. Added by change `add-uph-performance-page`.
- `UPH_PERFORMANCE_JOB_TIMEOUT_SECONDS`: Maximum seconds a single RQ uph-performance job may run before the worker kills it. Default 1800 s, matching the other `BaseChunkedDuckDBJob` worker timeouts. Added by change `add-uph-performance-page`.

**Worker env-var parity:** The new `mes-dashboard-uph-performance-worker.service` systemd unit MUST export the same `UPH_PERFORMANCE_*` env set as gunicorn (at minimum: `UPH_PERFORMANCE_USE_UNIFIED_JOB`, `UPH_PERFORMANCE_WORKER_QUEUE`, `UPH_PERFORMANCE_JOB_TIMEOUT_SECONDS`). Env-var drift silently breaks job-type registration parity. Added by change `add-uph-performance-page`.

**No dedicated `UPH_PERFORMANCE_SPOOL_DIR`/`_SPOOL_TTL`**: mirrors `production-achievement-async-spool`'s precedent — reuses the shared `QUERY_SPOOL_DIR` base directory (namespace subdirectory `uph_performance/`) and the shared `CACHE_TTL_DATASET` default for Redis spool-metadata TTL. Introduce a dedicated TTL var as a follow-up contract change only if the shared default proves unsuitable.
