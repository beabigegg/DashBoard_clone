...
| DOWNTIME_USE_UNIFIED_JOB | feature | all | no | no | off | off | application-team | off/on or false/true or 0/1; selects unified DowntimeJob path (on) vs legacy _bridge_jobid Path B pd.merge (off). always_async=True; async unavailable → 503 (no sync fallback). Restart required. | yes | uses default off |
