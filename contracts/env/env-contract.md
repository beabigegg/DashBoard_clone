...
- `DOWNTIME_ANALYSIS_PREWARM_MONTHS`: Number of calendar months of downtime-analysis data to load into the persistent DuckDB cache at startup. Background thread starts 10s after worker boot; `0` disables entirely. Default 3 months. Added by change `downtime-analysis-duckdb-cache`.
