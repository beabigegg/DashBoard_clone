...
| QUERY_TOOL_USE_RQ | feature | all | no | no | off | off | application-team | off/on or false/true or 0/1; enables RQ async dispatch for query-tool equipment-period endpoint when classify_query_cost returns ASYNC (date span ≥ 30 days). When on + oversized query → 202+job_id; else inline. Restart required. | yes | uses default off |
