...
| `uph_performance_routes.py` | All JSON API endpoints — **Type B** (spool-hit 200 / spool-miss 202 for `POST /api/uph-performance/spool`; client polls `GET /api/uph-performance/spool/status?query_id=`); fine-filter/aggregate views (`GET /filter-options`, `/trend`, `/ranking`, `/detail`) served from `uph_performance` DuckDB spool only (no Oracle re-query post-spool; 410 `CACHE_EXPIRED` on spool miss). `GET /product-filter-options` is Oracle-free (container_filter_cache), no `query_id` required. Restricted to `GDBA`/`GWBA` equipment families only (business-rules.md UPH-02). `PARAMETER_NAME` selected by family (`GDBA`→`BondUPH`, `GWBA`→`fHCM_UPH`, UPH-03); `PARAMETER_VALUE` used raw, no scale conversion (UPH-04). DB/WB label derived via `workcenter_groups` mapping (UPH-05, NOT equipment-ID-prefix enumeration — mirrors EA-07 precedent). `LAST_UPDATE_TIME` mandatory index filter, ≤6h chunked windows (UPH-01, mirrors EA-03). `_ALLOWED_NAMESPACES` must include `"uph_performance"`. |
