...
| `downtime_analysis_routes.py` | All JSON API endpoints — **Type A** (sync re-query on 410); `GET /api/downtime-analysis/options` → `{workcenter_groups[], families[], resources[], package_groups[], big_categories[], reasons[]}`; `POST /api/downtime-analysis/query` → Oracle query → `{query_id, summary: DowntimeKpiShape, daily_trend[], big_category[], top_reasons[]}` (spool namespace `downtime_analysis_*`, cache key includes `DOWNTIME_BRIDGE_VERSION`); `GET /api/downtime-analysis/view?query_id=&granularity=` → DuckDB regroup (no Oracle; 410 on spool miss); `GET /api/downtime-analysis/equipment-detail?query_id=&big_category=(opt)&status_types=(opt,CSV)` → `{equipment_detail: EquipmentDetailRow[]}` (wrapper key is `equipment_detail`); `GET /api/downtime-analysis/event-detail?query_id=&page=&page_size=&big_category=(opt)&status_types=(opt,CSV)&resource_id=(opt)` → `{events: paginated EventDetailRow[]}` (wrapper key is `events`) with nullable `JobEnrichment` (null → `match_source='none'`). Optional filter params apply pandas `.isin()` on in-memory `events_df`; no Oracle re-query; omitting all returns unfiltered response (backward compatible). `status_types` parsed by `_csv_param()` (CSV convention). DA-01..DA-06 apply. |
