...
### Removed
- production-achievement-oracle-plan-source: `GET/PUT /api/production-achievement/daily-plans`, `POST /api/production-achievement/daily-plans/import/preview`, `POST /api/production-achievement/daily-plans/import/confirm` (targets are now Oracle-sourced, business-rules.md PA-11). Removed schemas: `ProductionAchievementDailyPlanRow`/`Response`, `ProductionAchievementDailyPlanMapEntry`, and the 6 `ProductionAchievementDailyPlanImport*` schemas.
### Added
- production-achievement-oracle-plan-source: `GET /api/production-achievement/report` gains an optional `refresh_plan` bool param that bypasses the Oracle plan/target cache independently of `force_refresh` (implied by `force_refresh=true`). Additive/backward-compatible; documented alongside the pre-existing (previously undocumented) `force_refresh` param.

## [data 1.42.0] — 2026-07-16
### Changed (BREAKING)
- production-achievement-oracle-plan-source: §3.28.4 envelope's `plan_map` (§3.34) replaces `daily_plan_map` — Oracle-sourced, date-indexed `(output_date, plan_package_group) -> {planqty_input, planqty_output}`, no station dimension (business-rules.md PA-11). §3.31 `production_achievement_workcenter_merge_map` gains a `plan_source_side` column (PA-20, idempotent ALTER+backfill + explicit seed values). §3.32 (`production_achievement_daily_plans`) marked DEPRECATED — table not dropped (rollback policy) but no longer read/written.
### Added
- production-achievement-oracle-plan-source: New §3.34 `plan_map` shape (replaces the old `daily_plan_map` content at the same section number).
- production-achievement-oracle-plan-source: §3.30 amended — `package_lf_map` (§3.33) is no longer sourced from the MySQL table alone; it now overlays manual rows on top of an Oracle default layer (`DWH.MES_WIP_OUTPUTPLAN_DETAIL`). Corrected the `MYSQL_OPS_ENABLED=false` degrade claim: `package_lf_map` no longer empties in that case (it falls back to the Oracle default layer); it's empty only if BOTH the MySQL table AND the Oracle read degrade.

## [business 1.51.0] — 2026-07-16
### Changed
- production-achievement-oracle-plan-source: Rewrote PA-11 (target/plan source is now Oracle `MES_WIP_OUTPUTPLAN`, replacing the Excel-imported `production_achievement_daily_plans` table), PA-12 (daily denominator reads the Oracle-sourced `plan_map` on the exact day, not a static reused-daily value), PA-13 (cumulative plan is a REAL per-day `SUM(plan_map)`, replacing the `daily_plan_qty × elapsed_days` approximation). Tombstoned PA-16 (每日計畫 Excel 匯入 — REMOVED, superseded by PA-11; table/endpoints removed, row kept as historical record). PA-11 further documents the `refresh_plan` manual-refresh escape hatch (independent of `force_refresh`, see api 1.44.0) so 重新查詢 refreshes the plan-cache denominator, not just actual-output. PA-09 amended — D1's `package_lf_map` default is no longer 100% manually maintained; `get_package_lf_map()` now starts from `DWH.MES_WIP_OUTPUTPLAN_DETAIL`'s full PACKAGE→PACKAGE_REPORT reference table (new `get_oracle_package_lf_map()`, cached globally, TTL `CACHE_TTL_PRODUCTION_ACHIEVEMENT_PACKAGE_LF_ORACLE`, degrades to {}/last-known-good on Oracle failure) as the default, then overlays manual `production_achievement_package_lf_map` rows on top (manual always wins). D1 is now a genuinely sparse exceptions-only override layer instead of needing a row for every Oracle-known code. `GET /report`'s `refresh_plan=true` now also bypasses this new cache. The redundant 6-row manual seed added earlier this session (uncommitted, in `scripts/sql/production_achievement_tables.sql`) was reverted — confirmed via a live Oracle query that all 6 codes now resolve identically through the dynamic sync. **Column correction (caught by user review):** both this D1 sync query and PA-11's own plan query originally read `MES_WIP_OUTPUTPLAN_DETAIL.PACKAGEGROUP` — a genuine bug, since that is a different, coarser internal planning grouping that diverges from the actually-correct reporting column `PACKAGE_REPORT` for several raw codes (e.g. `SOT-23 CU` incorrectly merged into `SOT-23`; `SOT-353`/`SOT-363`/`SOT-363 CU` incorrectly merged into one group). Both queries (`production_achievement_package_lf_oracle.sql`, `production_achievement_plan.sql`) now read `PACKAGE_REPORT`; verified via a 3-month real-data sample that this creates no fan-out risk in the client's plan join. **Seed data correction (same review):** the manual DDL seed's `SOT23-5L`/`SOT23-6L` → `'SOT23-5L/6L'` (no hyphen) row-pair was REMOVED — it never matched Oracle's real `PACKAGE_REPORT` value `'SOT-23-5L/6L'` (hyphenated); a prior live-DB edit had already silently corrected only the `SOT23-6L` row, leaving the two disagreeing with each other. Both raw codes are now covered correctly by D1's Oracle default layer with no manual row needed. PA-09's "4 confirmed merges" documentation reduced to 3 (7 seed rows, was 9).
### Added
- production-achievement-oracle-plan-source: Added PA-20 (station Input/Output plan-column routing via `plan_source_side`, derived from 025.txt's station column order) and PA-21 (per-shift target derivation `shift_plan_qty = CEIL(daily_plan_qty / 2)` and per-shift achievement rate — fixes a latent bug where the D/N shift chart divided each shift's actual by the FULL daily plan instead of a per-shift target). Added PA-22 (actual-output unit correction) — a genuine unit-mismatch bug found after PA-11 shipped: Oracle's `plan_map` values are natively K (thousands of pcs, confirmed via 025.txt and live-data verification), while `pa_rollup.actual_output_qty` (both 產出/`TRACKOUTQTY` and 轉出/moveout `QTY`) is raw pcs — every achievement rate and displayed actual-output number was off by ~1000×. Fixed by dividing by `1000.0` in `useProductionAchievementDuckDB.ts`'s `computeDailyView`/`computeCumulativeView`/trend queries (view layer only, not `pa_rollup_raw`/`pa_rollup`, to avoid breaking `test_frontend_production_achievement_parity.py`'s unrelated grouping-correctness parity gate). Frontend labels gained a `(K)` suffix so the ~1000× change in displayed numbers is visible, not silent.

## [data 1.41.0] — 2026-07-15
### Added
- production-achievement-moveout: §3.31 `production_achievement_workcenter_merge_map` gains a `parent_group` column (PA-19, 大項/子站 two-layer; idempotent ALTER+backfill migration). New §3.28.5 `source` variants — the `moveout` source uses the `production_achievement_moveout` spool namespace with a `(output_date, shift_code, raw_workcenter_group, PACKAGE_LF)`-grain parquet (`raw_workcenter_group` instead of `SPECNAME`) and an empty `spec_workcenter_map`. §3.28.4 envelope gains `data.source` and a `parent_group` field on each `workcenter_merge_map` row. `_PA_SPOOL_SCHEMA_VERSION` 2→3.

## [business 1.50.0] — 2026-07-15
### Added
- production-achievement-settings-gate: Added PA-17 (settings-navigation permission pre-check). Client-side UX gate only — the 設定 button now checks `GET /api/production-achievement/permissions/me` before navigating and blocks with a message on deny/network-failure. Does not change `/production-achievement-settings`'s route-level `visibilityPolicy` (still `released_or_admin`, still readable via direct URL) — the actual write-enforcement boundary (PA-09/PA-10/PA-11 write endpoints, fail-closed on 403) is unchanged.
- production-achievement-moveout: Added PA-18 (轉出/`DW_MES_HM_LOTMOVEOUT` data source, TAB-switched with 產出 — `source` param on `/report`, `FROMWORKCENTER` grouping, trusts `SHIFTNAME`, `SUM(QTY)`, `PACKAGE_LF` reverse-lookup approximation, rework (`OWNERNAME NOT LIKE '重工%'`, validated vs Live 月轉出 to +0.13% — supersedes the original GA%/GC% container-prefix simplification that zeroed the back-end 大項) + same-station + 電鍍退火 exclusions, distinct spool namespace/job on the same RQ queue) and PA-19 (station 大項/子站 two-layer taxonomy — 電鍍={掛鍍/條鍍/滾鍍/委外}, 切割={切割/PKG_SAW}; `parent_group` column; DISTINCT-parent dropdown; Excel-style expanded detail; 委外=BANDL+TOTAI; TCT/MA/IST/補鍍 excluded).
### Changed
- production-achievement-moveout: PA-05 supplement — the 產出 query gains a GA%/GC% container-prefix rework filter via a `DW_MES_CONTAINER` INNER JOIN. Correcting change: the existing 焊接_DB/焊接_WB/成型 numbers can drop by the removed non-GA/GC volume.

## [api 1.43.0] — 2026-07-15
### Added
- production-achievement-settings-gate: New self-check endpoint `GET /api/production-achievement/permissions/me` (`login_required` only, deliberately NOT gated by `can_edit_targets` itself — a non-whitelisted user must still be able to ask the question). Returns `{can_edit_targets: boolean}` for the current session user via the existing `core.permissions.can_edit_targets()` check. Lets the `/production-achievement` report page's frontend pre-check permission before navigating to `/production-achievement-settings`, instead of only discovering the answer via a 403 on the first write inside that page. New schemas: `ProductionAchievementOwnPermissionData`, `ProductionAchievementOwnPermissionResponse`.
- production-achievement-moveout: `GET /api/production-achievement/report` gains an optional `source` query param (`output` default \| `moveout`, PA-18). `moveout` routes to the distinct `production_achievement_moveout` spool namespace + `production-achievement-moveout` job (same RQ queue) and returns an empty `spec_workcenter_map`. The 200 spool-hit envelope's inline `workcenter_merge_map` array gains a `parent_group` field (PA-19). `production_achievement_moveout` added to the spool-namespace enum.
...
### Changed
- production-achievement-overhaul: PA-06's implementation-locus note extended (additive clarification, no behavior change) to describe the workcenter_group-only rollup as Stage 1 of a new two-stage client-side pipeline; see ADR-0016's Extension section for the full current join chain.

## [api 1.41.0] — 2026-07-14
### Changed (BREAKING)
- production-achievement-overhaul: `GET /api/production-achievement/report` `ProductionAchievementReportResponse` redefined in place again (2→5 inline arrays: adds `package_lf_map`, `workcenter_merge_map`, `daily_plan_map` alongside unchanged `spec_workcenter_map`/`targets_map`); spool grain widens to `(output_date, shift_code, SPECNAME, PACKAGE_LF)`, `_PA_SPOOL_SCHEMA_VERSION` 1→2. BREAKING under the same schema name — no deprecation window (same pre-launch/atomic-PR/sole-consumer exception precedent as `production-achievement-async-spool`).
### Added
- production-achievement-overhaul: Ten new additive endpoints for 3 new MySQL CRUD tables plus 2 read-only "known values" helper endpoints: `GET/PUT/DELETE /api/production-achievement/package-lf-map[/{raw}]`, `GET/PUT/DELETE /api/production-achievement/workcenter-merge-map[/{raw}]`, `GET/PUT /api/production-achievement/daily-plans`, `GET /api/production-achievement/known-package-lf-values`, `GET /api/production-achievement/known-workcenter-groups` (the last added post-review per interaction-design.md OD-8) — all gated by the existing `can_edit_targets` permission verbatim, no new permission system. `GET /filter-options`, `GET/PUT /targets`, and the admin permission endpoints unchanged. New envelope-wrapper response schemas `ProductionAchievementPackageLfMapResponse`, `ProductionAchievementWorkcenterMergeMapResponse`, `ProductionAchievementDailyPlanResponse`, `ProductionAchievementKnownPackageLfValuesResponse`, `ProductionAchievementKnownWorkcenterGroupsResponse` (each `{success, data, meta}`, matching the universal envelope convention used by every other endpoint in this contract — corrected post-review from an initial bare-array-notation draft that omitted the envelope); `data` on the first three references the row-shape schemas `ProductionAchievementPackageLfMapRow`, `ProductionAchievementWorkcenterMergeMapRow`, `ProductionAchievementDailyPlanRow` (Tier-A) as `Row[]`, and on the last two references new `...Data` schemas (`package_lf_values`/`raw_workcenter_groups` string arrays); new shared `ProductionAchievementResponseMeta`. Plus supporting inline-map entry schemas (`ProductionAchievementSpecWorkcenterMapRow`, `ProductionAchievementTargetsMapEntry`, `ProductionAchievementPackageLfMapEntry`, `ProductionAchievementWorkcenterMergeMapEntry`, `ProductionAchievementDailyPlanMapEntry`) referenced by the redefined `ProductionAchievementReportResponse` (Tier-B, kept envelope-tolerant — only `success` required — to match its existing error-sample-capture convention).
...
### Added
- production-achievement-overhaul: New §3.30 (`production_achievement_package_lf_map` MySQL table — D1 sparse/fallback-to-self default). New §3.31 (`production_achievement_workcenter_merge_map` MySQL table — D2 explicit-inclusion/exclude-by-absence default; explicitly the OPPOSITE of §3.30). New §3.32 (`production_achievement_daily_plans` MySQL table — keyed `(workcenter_group, package_lf_group)`, no shift dimension; independent of and additive to §3.26/`production_achievement_targets`). New §3.33 (`package_lf_map`/`workcenter_merge_map` inline array shapes) and §3.34 (`daily_plan_map` inline array shape). §3.25/§3.26/§3.27/§3.28.2/§3.28.3 unchanged. See business-rules.md PA-09..PA-15 and ADR-0016's Extension section (production-achievement-overhaul) for the full current client-side join chain.
...
### Changed
- production-achievement-overhaul: `frontend/src/production-achievement/style.css` row annotated with the new mode-switch/chart-token rules (see `[css 1.16.0]`).
### Added
...
### Added
- production-achievement-async-spool: `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB` (default `on`; unlike sibling `*_USE_UNIFIED_JOB` flags, `off` is a pure kill switch — no legacy code path exists pre-launch); new `## Async Worker — Production Achievement Query` section (`PRODUCTION_ACHIEVEMENT_WORKER_QUEUE` default `production-achievement-query`, `PRODUCTION_ACHIEVEMENT_JOB_TIMEOUT_SECONDS` default `1800`); added to `## Worker Feature-Flag Env-Var Parity` cross-cutting list. `.env.example.template` and root `.env.example` updated.

## [business 1.45.0] — 2026-07-08
### Changed
- production-achievement-async-spool: New PA-08 rule (async spool routing, date-range-only canonical spool key, `PRODUCTION_ACHIEVEMENT_USE_UNIFIED_JOB` flag semantics). PA-06/PA-07 rows gain an "Implementation locus" clause documenting the backend→frontend (DuckDB-WASM) computation-location move (ADR-0016); business semantics (grouping key, formula, null/zero handling) unchanged. Three new Decision Table rows for the async routing branches. `build_achievement_rows()` retained as the test-only golden reference for the dual-tier parity gate.

## [ci 1.3.36] — 2026-07-08
### Added
- production-achievement-async-spool: Finalized Gate Compatibility Note (`ci-cd-gatekeeper`) for the new `production_achievement` spool namespace + new `mes-dashboard-production-achievement-worker.service` systemd unit. Adds two new Tier-1 required steps: `worker-env-parity-static` (`backend-tests.yml` — greps `deploy/*.service` for a hardcoded `*_USE_UNIFIED_JOB`/`*_USE_RQ` override, automating the env-contract §Worker Feature-Flag Env-Var Parity check) and `production-achievement-async-e2e` (`frontend-tests.yml`, mirrors `resource-history-async-e2e`). Notes that `tests/playwright/production-achievement.spec.js` must be updated in-place for the new async report contract. `stress-soak-report.md` sign-off required before the worker systemd unit is first started (adapted "required before flag promotion" precedent for a default-`on` flag with no gradual rollout). Full gate plan: `specs/changes/production-achievement-async-spool/ci-gates.md`. No new workflow file, no gate-tier change.
