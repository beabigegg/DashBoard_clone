...
| `ai_routes.py` | POST /api/ai/query — natural language query; pipeline selected by AI_MODE env var (`text2sql` default: classify → generate SQL → execute → summarize; `function`: 3-round function call pipeline); request body: `{question}`; response data: `{answer, chart_data, query_used, params_used, suggestions, sql_used, tool_trace}` — `sql_used` (string\|null) contains the generated SQL, `tool_trace` (array of `{step, function, summary, error?}`) contains execution steps; gated by AI_QUERY_ENABLED feature flag |
