...
| `hold_history_routes.py` | All JSON API endpoints — **Type A** (sync re-query on 410): view miss returns 410 `cache_expired`; client re-triggers `execute_primary_query()` synchronously; `duration` payload shape: `{ items: [{range, count, qty, pct}], avgReleasedHours, avgOnHoldHours, maxReleasedHours, maxOnHoldHours }`; each trend day includes `repeatQualityHoldQty` per hold-type section (non-breaking additive fields); **`POST /api/hold-history/today-snapshot`**: single-call today-mode snapshot, inputs `{ hold_type, record_type ("on_hold"\|"new"\|"release", CSV), reason, duration_range, page, per_page }`, response `{ query_id, summary: { onHoldTotalCount, onHoldTotalQty, todayNewQty, todayReleaseQty, todayFutureHoldQty, onHoldAvgHours, onHoldMaxHours }, reason_pareto, duration, list }`, no trend field; cache namespace `hold_today:*` TTL 60 s; DB unavailable → HTTP 503 `service_unavailable` with detail `"database_unavailable"`; truncated payload adds `_meta: { truncated, total_before_limit, limit_applied }` |
