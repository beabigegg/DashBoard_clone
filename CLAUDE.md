...
- Phase status: Phase 1a (`core/`), 1b (`shared-composables/`), 1c (`shared-ui/`), Phase 2 (`admin-shared/`, `resource-shared/`, `wip-shared/`) — all complete. Phase 3 (per-app) in progress. Remaining JS apps: `admin-dashboard`, `admin-performance`, `admin-user-usage-kpi`, `anomaly-overview`, `material-trace`, `mid-section-defect`, `portal`, `portal-shell`, `tables`.
