...
- **`_SCHEMA_VERSION` in the spool cache key is an automated zero-downtime alternative to manual `rm` for parquet schema-breaking changes**: embed a module-level `_SCHEMA_VERSION` integer in the query-id hash at write time (alongside any bridge-version constant). A redeploy that bumps `_SCHEMA_VERSION` orphans all in-flight parquets by key miss; readers rewrite without a manual `rm -f`. Pin the bump in the same commit as the SQL/column change. This does not replace the `rm` runbook for immediate rollback; it replaces it for forward-deploys. Pattern: `downtime_analysis_cache.py::_SCHEMA_VERSION`; `downtime_analysis_service.py::make_raw_spool_query_id`. Evidence: `downtime-browser-duckdb` — `data-shape-contract.md §3.13`; `ci-gates.md §Rollback Policy item 3`.
- **Multi-parquet spool services must validate all interdependent files atomically before serving any**: when a service writes two parquets that a browser DuckDB-WASM join requires (e.g., `base_events` + `job_bridge`), the read path must check both are present in one pass and raise `RuntimeError` (→ HTTP 500) if either is missing or expired. Never serve a partial result from only the base file — the browser JOIN will silently return an empty dataset with no observable error. Pattern: `downtime_analysis_service.py::query_downtime_dataset_raw` (DA-11 atomicity guard). Evidence: `downtime-browser-duckdb` — `TestTwoParquetAtomicity::test_base_hit_jobs_miss_raises_loudly`; `contracts/business/business-rules.md §DA-11`.
- **DuckDB prewarm spool TTL must use a per-service env var, not `CACHE_TTL_DATASET`**: `CACHE_TTL_DATASET` (7200 s / 2h) is sized for fast-expiring Oracle query caches (hold/reject/yield_alert) and is not appropriate for long-lived DuckDB prewarm spool files. Any service that adds a DuckDB prewarm spool must define its own env-var override (e.g., `RESOURCE_HISTORY_SPOOL_TTL`, `DOWNTIME_ANALYSIS_CACHE_TTL`) defaulting to 72000 s (20h), and pin the default with a `test_env_contract.py` assertion. Using `CACHE_TTL_DATASET` directly causes prewarm files to expire within one shift cycle, forcing repeated Oracle re-queries that negate the prewarm benefit. Pattern: `resource_dataset_cache.py:35`, `downtime_analysis_cache.py:37`; `contracts/env/env-contract.md §Cache Tuning — Resource History`. Evidence: `unify-duckdb-prewarm-rq`.
...
- **Use `ast.parse()` to prove absence of a removed startup side-effect, not mock.** When a daemon thread call or startup side-effect is removed from `app.py`, a mock-based test cannot prove absence — if the call were accidentally re-added, mocks would simply run it normally and still pass. Use `ast.parse(source)` and walk `ast.Call` nodes to assert the removed function name does NOT appear anywhere in the file. This permanently blocks re-introduction without a test failure. Pattern: `tests/test_app_startup.py::TestDaemonPrewarmRemovedFromApp` (asserts `start_duckdb_prewarm` and `start_downtime_prewarm` absent from `app.py`). Apply whenever any startup call is removed from `app.py`. Evidence: `unify-duckdb-prewarm-rq`.
