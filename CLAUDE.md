...
- `vi.mock('...file.js')` static mock calls resolve correctly after renaming `file.js → file.ts` — Vite's mock resolution handles `.js` → `.ts` transparently. Do NOT update `.js` specifiers in static `vi.mock()` calls during migration. Dynamic `import('...file.js')` does NOT benefit from this — drop the `.js` extension when renaming the target file. Evidence: `yield-alert-center-utils.test.js`, `yield-alert-abort.test.js` (migrate-yield-alert-center-ts).
- When a TS file wraps a not-yet-migrated `.js` file: declare interface locally → `// @ts-expect-error <phase note>` → cast as `DeclaredInterface`. Prefer over `as any`. Use double-cast (`as unknown as DeclaredInterface`) when suppressing a full return expression.
- Audit `.js` import specifiers inside SFCs being migrated. Drop the extension entirely (don't rename to `.ts`) — TypeScript/Vite resolve `.ts` automatically, preventing future stale specifier accumulation.
- Each feature app's `index.html` references `./main.js` as Vite entry. Do NOT update it during Phase 3 migrations — Vite resolves `main.ts` at build time; this is an intentional pre-existing pattern across all apps.
- echarts callback parameters (`params` in formatter/tooltip) lack precise types. Annotate with `// TODO: type echarts callback` and do not block migration — known library gap.
- When migrating an SFC with a paired `node --test` legacy test using `readSource` + `assert.match`, audit every regex for JS-specific syntax. Replace with prefix/looser forms that survive both JS and TS (e.g., `/const activeFilter = ref/` instead of `/ref\(null\)/`).
- SFC-paired unit tests require `frontend/vitest.config.js` `include` to list `src/**/*.test.ts`. The default pattern only covers `tests/**/*.test.js`, so a test placed next to its SFC (e.g., `frontend/src/shared-ui/components/__tests__/MultiSelect.test.ts`) is silently skipped — vitest reports 0 failures even when the file is broken. Add the `src/**/*.test.ts` entry once; subsequent SFC-paired tests need no config change. Evidence: `fix-prod-history-multiselect-filter`.
- Every Phase 3 per-app migration must bump `contracts/ci/ci-gate-contract.md` (patch version) with a `### frontend-type-check scope expansion` note documenting the before/after `tsconfig.json` `include` state, and add a matching entry to `contracts/CHANGELOG.md`. This step is required in the same PR as the `tsconfig.json` change. Evidence: 8 consecutive scope-expansion entries, ci 1.3.1 through 1.3.13 (`migrate-material-trace-ts`).
