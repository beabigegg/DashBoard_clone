...
Defines the shared async job service that wraps RQ for long-running query workloads: enqueue, status query, progress updates, completion marking, and worker lifecycle for reject-history, yield-alert, production-history, msd-lineage, trace-lineage, and mid-section-defect job pipelines.
## Requirements
