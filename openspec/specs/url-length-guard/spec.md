## Purpose
Define stable requirements for the URL length guard — a transparent mechanism that prevents browser and server `400 Bad Request` errors when filter state exceeds safe URL length limits. The guard stores overflow state in sessionStorage and a companion module provides cross-page navigation state transfer.
