...
Stress probes for async RQ job lifecycle: polling, queue saturation, timeout, retry, spool deduplication, queue depth monitoring, and end-to-end data integrity for async-path queries.
...
- **THEN** the total job executions SHALL be <= 30 (10 jobs x 3 attempts max)
- **THEN** the queue SHALL drain completely within `10 x (1800 + 30 + 60)` seconds worst case
