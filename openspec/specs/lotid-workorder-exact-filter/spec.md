# lotid-workorder-exact-filter Specification

## Purpose
Define requirements for exact-match filtering on LOTID and WORKORDER fields in WIP snapshot queries. These fields SHALL use pre-built value indexes (O(1) dict lookup) rather than substring/fuzzy matching, ensuring correctness and performance when large filter sets are applied.
