...
The following pre-existing bugs were uncovered during triage (not introduced by this change):
...
| Missing input validation on three fuzz-flagged endpoints | `/api/reject-history/options`, `/api/hold-overview/summary`, `/api/wip/overview/summary` silently accept SQL-injection, 100k-char strings, null bytes, inverted dates, negative pagination, Unicode/emoji, CSV-injection, and whitespace-only and return `200` with empty arrays instead of `400 VALIDATION_ERROR`. 24 fuzz cases marked `xfail(strict=False)` in `test_fuzz_routes.py`. Uncovered during post-review tightening (Round 1). | [fix-missing-fuzz-validation](../fix-missing-fuzz-validation/) |
| `@map_service_errors` decorator swallows `DatabaseDegradedError` subclasses | Every `/api/query-tool/*` route under pool-exhaustion or circuit-open returns `500 INTERNAL_ERROR` with no `Retry-After` instead of the wired `503 DB_POOL_EXHAUSTED` / `503 CIRCUIT_BREAKER_OPEN`. The catch-all `except Exception` at [query_tool_routes.py:83-89](../../src/mes_dashboard/routes/query_tool_routes.py#L83-L89) intercepts degraded exceptions before they reach the app-level error handler. Pinned by 2 tests in `test_oracle_error_path.py::TestMapServiceErrorsSwallowsDegradedErrors`. Uncovered during post-review tightening (Round 2, C1). | [fix-map-service-errors-propagates-degraded](../fix-map-service-errors-propagates-degraded/) |
| No app-level handler for `oracledb.DatabaseError` | Raw ORA-01017 (invalid credentials), ORA-12514 (listener unknown), ORA-01555 (snapshot too old), ORA-03113 / 03135 (connection lost) all collapse to `500 INTERNAL_ERROR` because there is no `@app.errorhandler(oracledb.DatabaseError)` — the existing `_extract_ora_code()` helper is never dispatched at the HTTP boundary. SREs cannot distinguish config problems from capacity problems from transient infra via response codes. Pinned by 3 tests in `test_oracle_error_path.py::TestUnmappedOraCodesFallThroughToGenericHandler`. Uncovered during post-review tightening (Round 2, C1). | [fix-oracle-ora-code-mapping](../fix-oracle-ora-code-mapping/) |
