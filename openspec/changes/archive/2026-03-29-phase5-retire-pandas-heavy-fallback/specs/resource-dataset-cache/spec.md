...
### Requirement: Resource dataset cache pandas derive functions
**Reason**: `_derive_summary()`, `_derive_kpi()`, `_derive_trend()`, `_derive_heatmap()`, `_derive_comparison()`, `_derive_detail()` are replaced by DuckDB SQL runtime in `apply_view()`. No callers remain after `execute_primary_query()` refactoring.
