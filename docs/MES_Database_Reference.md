...
| # | 表名 | 用途 | 数据量 |
|---|------|------|--------|
| 1 | `DW_MES_CONTAINER` | 容器/批次主檔 - 目前在製容器狀態、數量與流程資訊 | 5,186,511 |
| 2 | `DW_MES_HM_LOTMOVEOUT` | 批次出站事件歷史表 - 出站/移出交易 | 48,384,700 |
| 3 | `DW_MES_HOLDRELEASEHISTORY` | Hold/Release 歷史表 - 批次停工與解除紀錄 | 310,056 |
| 4 | `DW_MES_JOB` | 設備維修工單表 - 維修工單的當前狀態與流程 | 1,239,996 |
| 5 | `DW_MES_JOBTXNHISTORY` | 維修工單交易歷史表 - 工單狀態變更紀錄 | 9,490,696 |
| 6 | `DW_MES_LOTMATERIALSHISTORY` | 批次物料消耗歷史表 - 用料與批次關聯 | 17,707,306 |
| 7 | `DW_MES_LOTREJECTHISTORY` | 批次不良/報廢歷史表 - 不良原因與數量 | 15,682,978 |
| 8 | `DW_MES_LOTWIPDATAHISTORY` | 在製數據採集歷史表 - 製程量測/參數紀錄 | 77,198,702 |
| 9 | `DW_MES_LOTWIPHISTORY` | 在製流轉歷史表 - 批次進出站與流程軌跡 | 53,100,091 |
| 10 | `DW_MES_MAINTENANCE` | 設備保養/維護紀錄表 - 保養計畫與點檢數據 | 51,012,334 |
| 11 | `DW_MES_PARTREQUESTORDER` | 維修用料請求表 - 維修/設備零件請領 | 61,396 |
| 12 | `DW_MES_PJ_COMBINEDASSYLOTS` | 併批紀錄表 - 合批/合併批次關聯與數量資訊 | 1,956,064 |
| 13 | `DW_MES_RESOURCE` | 資源表 - 設備/載具等資源基本資料（OBJECTCATEGORY=ASSEMBLY 時，RESOURCENAME 為設備編號） | 90,620 |
| 14 | `DW_MES_RESOURCESTATUS` | 設備狀態變更歷史表 - 狀態切換與原因 | 65,174,816 |
| 15 | `DW_MES_RESOURCESTATUS_SHIFT` | 設備狀態班次彙總表 - 班次級狀態/工時 | 74,192,914 |
| 16 | `DW_MES_WIP` | 在製品現況表（含歷史累積）- 當前 WIP 狀態/數量 | 77,531,193 |
...
1. **大数据量表查询优化**
   - 以下表数据量较大，查询时务必添加时间范围限制：
     - `DW_MES_WIP`: 77,531,193 行
     - `DW_MES_LOTWIPDATAHISTORY`: 77,198,702 行
     - `DW_MES_RESOURCESTATUS_SHIFT`: 74,192,914 行
     - `DW_MES_RESOURCESTATUS`: 65,174,816 行
     - `DW_MES_LOTWIPHISTORY`: 53,100,091 行
     - `DW_MES_MAINTENANCE`: 51,012,334 行
     - `DW_MES_HM_LOTMOVEOUT`: 48,384,700 行
     - `DW_MES_LOTMATERIALSHISTORY`: 17,707,306 行
     - `DW_MES_LOTREJECTHISTORY`: 15,682,978 行
