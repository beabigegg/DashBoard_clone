...
class TestNoSrcFilesModified(unittest.TestCase):
    """AC-7 (response-shape-adr0007): no production source under src/ should
    be modified by this contract-annotation change.

    This test ensures the current HEAD diff does not touch src/ files.
    It is intentionally lenient: if git is unavailable (e.g. detached HEAD
    in CI sandbox), the test is skipped rather than failed.
    """

    def test_no_src_files_modified(self):
        """git diff --name-only HEAD must not list any path starting with src/."""
        import subprocess
        try:
        ...
        if result.returncode != 0:
            # Could be a detached HEAD or empty repo — skip gracefully
            self.skipTest(
                f"git diff returned non-zero ({result.returncode}): {result.stderr}"
            )
        ...
        self.assertEqual(
            src_paths,
            [],
            f"response-shape-adr0007 must not modify src/ files. "
            f"Found:\n" + "\n".join(src_paths),
        )
